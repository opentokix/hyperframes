...
export function sanitizeSvg(svg: string): string {
  ...
  out = replaceStable(out, /<style\b[\s\S]*?<\/style\s*>/gi, "");
  out = replaceStable(out, /<foreignObject\b[\s\S]*?<\/foreignObject\s*>/gi, "");
  out = replaceStable(out, /<foreignObject\b[^>]*\/>/gi, "");
  // Nesting leaves inert orphan close tags after the stable pass — drop them.
  out = out.replace(/<\/(?:script|style|foreignObject)\s*>/gi, "");
